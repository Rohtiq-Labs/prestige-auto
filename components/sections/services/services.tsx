...
export const Services = (): React.ReactNode => (
  <section id="services" className="relative bg-brand-bg-2" aria-label="Services">
    <div className="flex flex-col items-start justify-between gap-5 border-b border-brand-border px-6 pb-10 pt-[60px] md:flex-row md:items-end md:px-[60px] md:pb-20 md:pt-[120px]">
      ...
      <Reveal delay={2} className="max-w-[260px] text-right text-xs leading-[1.7] tracking-[0.06em] text-brand-gray md:text-[0.75rem]">
        Hover to explore our full suite of protective and enhancement services — each
        installed with uncompromising precision.
      </Reveal>
    </div>

    <div className="service-panels flex h-auto flex-col md:h-[500px] md:flex-row">
      {services.map((service) => (
        <div
          key={service.id}
          className="service-panel group relative flex-1 overflow-hidden border-b border-brand-border md:border-b-0 md:border-r md:last:border-r-0"
        >
          <div className="service-panel-bg absolute inset-0 opacity-0 transition-opacity duration-500" aria-hidden="true">
            <SiteImage
